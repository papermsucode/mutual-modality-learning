...
parser.add_argument('--dense_length', type=int, default=32, help='Num of frames for the dense sampling')
parser.add_argument('--dense_number', type=int, default=1, help='Num of dense samples')
parser.add_argument('--twice_sample', default=False, action="store_true", help='use twice sample for ensemble')
parser.add_argument('--random_sample', default=False, action="store_true", help='use random sample for ensemble')
    ...
    data_loader = torch.utils.data.DataLoader(
            TSNDataSet(root_path, [test_file] if test_file is not None else val_list, num_segments=this_test_segments,
                       ...
                       transform=torchvision.transforms.Compose([
                           ...
                           GroupNormalize(net.input_mean, net.input_std) if modality!='RGBDiff' else IdentityTransform(),
                       ]), dense_sample=args.dense_sample, dense_length=args.dense_length, dense_number=args.dense_number,
                       twice_sample=args.twice_sample, random_sample=args.random_sample),
    ...
    with torch.no_grad():
        ...
        if args.random_sample:
            if args.twice_sample:
                num_crop *= (args.dense_number+2)
            else:
                num_crop *= (args.dense_number+1)
        elif args.dense_sample:
            num_crop *= args.dense_number  # 10 clips for testing when using dense sample
