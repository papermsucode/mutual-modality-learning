...
class TSN(nn.Module):
    ...
    def _construct_flow_model(self, base_model):
        ...
        new_kernel_size = kernel_size[:1] + (2 * 5, ) + kernel_size[2:]
        new_kernels = params[0].data.mean(dim=1, keepdim=True).expand(new_kernel_size).contiguous()

        new_conv = nn.Conv2d(2 * 5, conv_layer.out_channels,
    ...
    def _construct_diff_model(self, base_model, keep_rgb=False):
        ...
        kernel_size = params[0].size()
        if not keep_rgb:
            new_kernel_size = kernel_size[:1] + (3 * 5,) + kernel_size[2:]
            new_kernels = params[0].data.mean(dim=1, keepdim=True).expand(new_kernel_size).contiguous()
        else:
            new_kernel_size = kernel_size[:1] + (3 * 5,) + kernel_size[2:]
            new_kernels = torch.cat((params[0].data, params[0].data.mean(dim=1, keepdim=True).expand(new_kernel_size).contiguous()),
                                    1)
            new_kernel_size = kernel_size[:1] + (3 + 3 * 5,) + kernel_size[2:]
    ...
    def _construct_rgb_model(self, base_model):
        ...
        new_conv = nn.Conv2d(3 * 1, conv_layer.out_channels,
                             conv_layer.kernel_size, conv_layer.stride, conv_layer.padding,
                             bias=True if len(params) == 2 else False)
        new_conv.weight.data = new_kernels
        if len(params) == 2:
